...
class CreditPaymentScreenState extends State<CreditPaymentScreen> {
  ...
  handlePayNow() {
    Navigator.of(context).push(
      CupertinoPageRoute (
          builder: (context) => const CreditPayNowScreen()
      ),
    );
  }
  ...
  handleDirectDebit() {
    Navigator.of(context).push(
      CupertinoPageRoute (
          builder: (context) => const CreditDirectDebitScreen()
      ),
    );
  }
  ...
  Widget depositFundsField() {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text('Payment Options', style:TextStyle(fontSize: fSize(18), fontWeight: FontWeight.w600, color: const Color(0xFF1A2831))),
          const CustomSpacer(size: 10),
          Text('Your account will be activated within (1) business\nday upon receipt of payment.',
              style:TextStyle(fontSize: fSize(12), fontWeight: FontWeight.w400, color: const Color(0xFF70828D))),
          const CustomSpacer(size: 20),
          paymentOptionField('Manually transfer from your bank', () => handleManuallyTransfer()),
          const CustomSpacer(size: 15),
          paymentOptionField('PayNow', () => handlePayNow()),
          const CustomSpacer(size: 15),
          paymentOptionField('Direct Debit Authorisation', () => handleDirectDebit()),
        ],
      ),
    );
  }
