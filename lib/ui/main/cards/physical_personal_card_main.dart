...
class PhysicalPersonalCardMain extends StatefulWidget {
  final cardData;
  final isTeam;
  final handleReload;
  const PhysicalPersonalCardMain(
      {Key? key, this.cardData, this.isTeam, this.handleReload})
      : super(key: key);

  @override
  PhysicalPersonalCardMainState createState() =>
      PhysicalPersonalCardMainState();
}

class PhysicalPersonalCardMainState extends State<PhysicalPersonalCardMain> {
  hScale(double scale) {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      print("===== Error : Physical Personal Card Transactions =====");
  ...
  Widget home() {
    return Material(
      ...
      SingleChildScrollView(
        ...
        CustomMainHeader(
            title:
                '${widget.cardData["accountName"] == null ? "" : widget.cardData["accountName"]}’s Card'),
  ...
  Widget eyeIconField() {
    return widget.cardData['status'] == "INACTIVE"
        ? Container(height: hScale(34))
        : Container(
            height: hScale(34),
            width: hScale(34),
            decoration: BoxDecoration(
